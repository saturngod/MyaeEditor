...
// MARK: - Display-equation block

/// A centered display equation. Its LaTeX source lives in the block's text.
/// Click to edit; empty blocks open the editor automatically.
struct EquationBlockView: View {
    ...
    var body: some View {
        Button { startEditing() } label: { content }
            ...
            .popover(isPresented: $editing, arrowEdge: .bottom) {
                MathEditor(latex: $draft, fontSize: displaySize) { latex in
                    block.text = NSAttributedString(string: latex.trimmingCharacters(in: .whitespaces))
                    document.markEdited()
                    editing = false
                }
            }
            .onAppear { if block.plainText.isEmpty { startEditing() } }
    }
    ...
    private var content: some View {
        if block.plainText.isEmpty {
            ...
            MathPreview(latex: block.plainText, fontSize: displaySize, background: false)
                .frame(maxWidth: .infinity)
        }
    }

    private func startEditing() {
        draft = block.plainText
        editing = true
    }
}
